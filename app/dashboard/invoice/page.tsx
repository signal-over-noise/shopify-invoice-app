...
export default function InvoiceFormPage() {
  ...
  const fetchOrderData = async () => {
    try {
      ...
      const endpoint = orderType === 'regular' 
      ...
      if (data.success) {
        ...
        handleError(new Error(data.error || 'Failed to fetch order'), 'fetchOrder');
      }
    } catch (error) {
  ...
  const convertOrderToInvoice = async (orderData: ShopifyOrder) => {
    ...
    const lineItems: InvoiceLineItem[] = orderData.line_items.map((item, index) => ({
      id: `${item.id || index}`,
    ...
    const invoiceData: InvoiceData = {
      ...
      customer: {
        ...
        address: extractAddress(orderData.shipping_address || orderData.billing_address)
      },
  ...
  const fetchSingleProductImage = async (productId: number, lineItemId: string) => {
    try {
  ...
  const updateLineItem = (index: number, field: keyof InvoiceLineItem, value: any) => {
    ...
    const total = subtotal + invoiceData.tax_amount + invoiceData.shipping_cost - invoiceData.discount_amount;
  ...
  const generateInvoice = async () => {
    ...
    try {
      ...
      if (!validation.isValid) {
        handleError(new Error(`Invalid invoice data: ${validation.errors.join(', ')}`), 'generatePDF');
        setSaveStatus('error');
        return;
      }
      ...
      a.href = url;
      a.download = `Invoice_${invoiceData.invoice_number.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`;
  ...
  const validateInvoiceData = (data: InvoiceData): { isValid: boolean; errors: string[] } => {
    const errors: string[] = [];
    ...
    if (!data.line_items.length) {
      ...
      const invalidItems = data.line_items.filter(item => 
        !item.title?.trim() || item.quantity <= 0 || item.price < 0
      );
      if (invalidItems.length > 0) {
        errors.push('All items must have a title, positive quantity, and non-negative price');
      }
    }
  ...
  const prepareInvoiceDataForPDF = async (data: InvoiceData): Promise<InvoiceData> => {
    const lineItemsWithImages = await Promise.all(
      data.line_items.map(async (item) => {
        if (item.product_id && productImages[item.product_id]) {
          const base64Image = await convertImageToBase64(productImages[item.product_id]);
          return {
            ...item,
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">
                  {order ? `From Order ${invoiceData.order_number}` : 'Custom Invoice'}
                </p>
              </div>
            </div>
            <button
      ...
      {Object.keys(errors).length > 0 && (
        <div className="w-full px-4 sm:px-6 lg:px-8 py-4 bg-red-50 border-b border-red-200">
          {Object.entries(errors).map(([context, message]) => (
            <div key={context} className="flex items-center justify-between mb-2 last:mb-0">
              <div className="flex">
      ...
      <div className="flex flex-col lg:flex-row h-[calc(100vh-4rem)]">
        ...
        <div className="w-full lg:w-1/2 overflow-y-auto border-r border-slate-200">
          <div className="p-4 sm:p-6 lg:p-8">
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) => updateInvoiceData('client_reference', e.target.value)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateInvoiceData('customer', { ...invoiceData.customer, email: e.target.value })}
                      className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 text-slate-900"
                    />
                  </div>
                </div>
              ...
              {/* Company Details */}
              <div className="bg-white rounded-lg border border-slate-200 p-6">
                <h2 className="text-lg font-medium text-slate-900 mb-4">Company Details</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateInvoiceData('company', { 
                        ...invoiceData.company, 
                        address: { ...invoiceData.company.address, line1: e.target.value }
                      })}
                      className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 text-slate-900"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateInvoiceData('company', { 
                        ...invoiceData.company, 
                        address: { ...invoiceData.company.address, city: e.target.value }
                      })}
                      className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 text-slate-900"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateInvoiceData('company', { 
                        ...invoiceData.company, 
                        address: { ...invoiceData.company.address, zip: e.target.value }
                      })}
                      className="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 text-slate-900"
                    />
                  </div>
              ...
              <div className="bg-white rounded-lg border border-slate-200 p-6">
                ...
                <textarea
                  ...
                  onChange={(e) => updateInvoiceData('delivery_terms', e.target.value)}
                  rows={3}
                      ...
                      <div className="flex items-start space-x-3 mb-3">
                        ...
                        <div className="flex-1">
                          <ProductSearchDropdown
                            ...
                            onChange={(value) => {
                              setProductSearchValues(prev => ({ ...prev, [item.id]: value }));
                            }}
                            onSelectProduct={(product, variant) => {
                              ...
                              setInvoiceData(prevData => {
                                ...
                                const newLineItems = prevData.line_items.map((lineItem, i) => {
                                  if (i === index) {
                                    return {
                                      ...lineItem,
                                ...
                                const subtotal = newLineItems.reduce((sum, lineItem) => sum + lineItem.total, 0);
                                const newTotal = subtotal + prevData.tax_amount + prevData.shipping_cost - prevData.discount_amount;
              ...
              <div className="bg-white rounded-lg border border-slate-200 p-6">
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-slate-900">{invoiceData.currency} {invoiceData.subtotal.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Tax:</span>
                    <span className="font-medium text-slate-900">{invoiceData.currency} {invoiceData.tax_amount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between border-t pt-3">
                    <span className="font-semibold text-slate-900 text-lg">Total:</span>
                    <span className="font-bold text-lg text-slate-900">{invoiceData.currency} {invoiceData.total.toFixed(2)}</span>
                  </div>
                </div>
              </div>
        ...
        <div className="w-full lg:w-1/2 bg-white overflow-hidden">
          <div className="h-full bg-white">
            <CleanPDFPreview invoiceData={invoiceData} productImages={productImages} />
          </div>
        </div>
      </div>
    </div>
